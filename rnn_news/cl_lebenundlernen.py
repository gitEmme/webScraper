...
def classify():
    ...
    for comment in client.spiegel.lebenundlernen.find(no_cursor_timeout=True):
        ...
        client.spiegel.lebenundlernen.update({'_id': comment['_id']}, {"$set": {'sentiment': sentiment}},upsert=True)
        client.spiegel.lebenundlernen.update({'_id': comment['_id']}, {"$set": {'sentimentNews': [p,neu,neg]}}, upsert=True)
        print(sentiment)
        print(res)
    gc.collect()
