...
def classify():
    ...
    for comment in client.spiegel.wissenschaft.find(no_cursor_timeout=True):
        ...
        client.spiegel.wissenschaft.update({'_id': comment['_id']}, {"$set": {'sentiment': sentiment}},upsert=True)
        client.spiegel.wissenschaft.update({'_id': comment['_id']}, {"$set": {'sentimentNews': [p,neu,neg]}}, upsert=True)
        print(sentiment)
        print(res)
    gc.collect()
