...
def rnn(trainset,testset):
    ...
    model.fit(x_train_pad, y_train,
              validation_split=0.05, epochs=100, batch_size=64, shuffle=True)
    result = model.evaluate(x_test_pad, y_test)
    print("Accuracy: {0:.2%}".format(result[1]))
    print(type(model))
    tf.keras.models.save_model(
        model,
        'zugsoft100model.hdf5',
        overwrite=True,
        include_optimizer=True
    )
    model.save_weights('zugsoft100net.h5')
...
def classify():
    ...
    for comment in client.spiegel.wissenschaft.find(no_cursor_timeout=True):
        ...
        client.spiegel.wissenschaft.update({'_id': comment['_id']}, {"$set": {'sentiment3': sentiment}}, upsert=True)
        client.spiegel.wissenschaft.update({'_id': comment['_id']}, {"$set": {'sentimentZugSoft': [p, neu, neg]}}, upsert=True)
        print(sentiment)
        print(res)
    gc.collect()
