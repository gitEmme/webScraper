...
def classify():
    ...
    for comment in client.spiegel.auto.find(no_cursor_timeout=True):
        ...
        client.spiegel.auto.update({'_id': comment['_id']}, {"$set": {'sentiment3': sentiment}}, upsert=True)
        client.spiegel.auto.update({'_id': comment['_id']}, {"$set": {'sentimentSoftZug': [p, neu, neg]}}, upsert=True)
        #print(sentiment)
        #print(res)
    gc.collect()
