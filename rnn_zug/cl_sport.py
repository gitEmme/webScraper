...
def classify():
    ...
    for comment in client.spiegel.sport.find(no_cursor_timeout=True):
        ...
        client.spiegel.sport.update({'_id': comment['_id']}, {"$set": {'sentiment2': sentiment}}, upsert=True)
        client.spiegel.sport.update({'_id': comment['_id']}, {"$set": {'sentimentZug': [p, neu, neg]}}, upsert=True)
        print(sentiment)
        print(res)
    gc.collect()
